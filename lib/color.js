var Color = (function(undefined) {
  ...
  function maxRGB(self) {
    return Math.max(self.r, self.g, self.b);
  }

  function minRGB(self) {
    return Math.min(self.r, self.g, self.b);
  }

  function midAttr(self) {
    return (self.r + self.g + self.b) - (maxRGB(self) + minRGB(self));
  }
  ...
  cache(proto, 'chroma', function() {
    return maxRGB(this) - minRGB(this);
  });
  ...
  cache(proto, 'lum', function() {
    return Math.floor(0.5 * (maxRGB(this) + minRGB(this)));
  });

  // -*- uncached methods -*- //
  proto.code = proto.toString = function() { return '#' + this.hex(); };

  proto.inspect = function() {
    return "Color.hex('"+this.code()+"')";
  };
